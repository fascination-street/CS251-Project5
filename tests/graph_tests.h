...
graph<int, int> tournamentGraph(int n) {
    graph<int, int> g;
    for (int i = 0; i < n; i++) {
        EXPECT_TRUE(g.addVertex(i));
    }
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < i; j++) {
            // Edge from i to j iff i > j
            EXPECT_TRUE(g.addEdge(i, j, i));
        }
    }
    return g;
}

graph<int, int> completeGraph(int n) {
    graph<int, int> g;
    for (int i = 0; i < n; i++) {
        EXPECT_TRUE(g.addVertex(i));
    }
...
TEST(Graph, CompleteEdgesStress) {
    int n = 400;
    graph<int, int> g = completeGraph(n);
    int weight;
...
TEST(Graph, NeighborsStress) {
    int n = 100;
    graph<int, int> g = tournamentGraph(n);
...
TEST(Graph, DefaultCopies) {
    graph<int, int> g = completeGraph(4);
